  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400">Track all generated links, QR codes, views, and suspicious attempts.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-6">
          {shares.map(share => (
            <div key={share.id} className="bg-dark-surface border border-dark-border rounded-xl p-6 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-4">
                  <div className="flex items-center gap-2 text-sm">
                    <div className="w-8 h-8 rounded-full bg-dark-bg border border-dark-border flex items-center justify-center text-blue-400">
                    ...
                    <div>
                      <p className="text-gray-300 font-medium">{share.views}</p>
                      <p className="text-gray-500 text-xs uppercase">Views</p>
                    </div>
                  </div>
                  
                  <div className="flex items-center gap-2 text-sm">
                    <div className="w-8 h-8 rounded-full bg-dark-bg border border-dark-border flex items-center justify-center text-red-400">
                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-white font-medium">{share.suspicious}</p>
                      <p className="text-red-400/80 text-xs uppercase">Suspicious Attempts</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-row md:flex-col gap-3 min-w-[140px]">
                <button 
                  ...
                  className="flex-1 py-2 px-4 rounded-lg text-sm font-medium border border-dark-border bg-dark-bg hover:bg-dark-border text-white transition-colors flex justify-center items-center gap-2"
                >
                   Copy Link
                </button>
                {share.status === 'Active' ? (
                  <button 
                    ...
                    className="flex-1 py-2 px-4 rounded-lg text-sm font-medium border border-red-500/30 bg-red-500/10 hover:bg-red-500/20 text-red-400 transition-colors flex justify-center items-center gap-2"
                  >
                     Revoke Access
                  </button>
                ) : (
                  <button 
                    disabled
                    className="flex-1 py-2 px-4 rounded-lg text-sm font-medium border border-dark-border bg-dark-bg text-gray-500 cursor-not-allowed opacity-50 flex justify-center items-center gap-2"
                  >
                     Revoked
                  </button>
                )}
